...
import json
import math
from transformers import MegatronBertConfig, MegatronBertModel,MegatronBertForMaskedLM, RoFormerModel,RoFormerForMaskedLM
...
class FillMaskPipeline():
    def __init__(self, model_name, device):
        ...
        if model_name == "IDEA-CCNL/Erlangshen-MegatronBert-1.3B":
            self.model = MegatronBertForMaskedLM.from_pretrained("IDEA-CCNL/Erlangshen-MegatronBert-1.3B").to(device)
        elif model_name == "IDEA-CCNL/Zhouwenwang-Unified-1.3B":
            self.model = RoFormerForMaskedLM.from_pretrained("IDEA-CCNL/Zhouwenwang-Unified-1.3B").to(device)
        else:
    ...
    def evaluate(self, inputs, targets, debug:bool=False):
        ...
        score_batch = []
        with torch.no_grad():
            # print(self.model(**input_tokens))
            if self.name == "IDEA-CCNL/Erlangshen-MegatronBert-1.3B":
