    ...
    for adj in adjs['group'][0]:
        for k in range(len(subj2)):
            for subj_pair in subj2[k]:
                ...
                prob_formats[1-1].append(todic(sentence = subj_pair[0]+"比"+subj_pair[1]+"更"+adj+"吗？", sentiment = 0, ans = prob_format1_verbalizer, topic = topics[k]))
                prob_formats[1-1].append(todic(sentence = subj_pair[1]+"比"+subj_pair[0]+"更"+adj+"吗？", sentiment = 1, ans = prob_format1_verbalizer, topic = topics[k]))
    ...
    for adj in adjs['group'][1]:
        for k in range(4):
            for subj in subj1[k][1]:
                ...
                prob_formats[2-1].append(todic(sentence = subj + "都很" + adj + "吗？", sentiment = 0, ans = prob_format2_verbalizer, topic = topics[k]))
            for t in range(2):
